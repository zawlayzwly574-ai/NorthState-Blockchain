...
**How to apply:** Use the portfolio query as the display source on both pages and format both canonical balances as USD. Serialize placement, admin adjustment, deposit approval, withdrawal approval, and settlement with the same per-user lock. Withdrawal approval must verify both the selected holding and canonical balance before changing status, then debit both atomically without clamping. Preserve existing records. A failed or pending portfolio read may keep previously cached data, but must otherwise fall back to zero and empty holdings rather than fabricated member funds.